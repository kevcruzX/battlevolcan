<?php
namespace CineramaCore\CPT\Shortcodes\BlogSlider;
...
class BlogSlider implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(
			array(
				'name'                      => esc_html__( 'Blog Slider', 'cinerama' ),
				'base'                      => $this->base,
				...
				'params'                    => array(
					array(
						...
						'value'       => array(
							esc_html__( 'Slider', 'cinerama' )            => 'slider',
							esc_html__( 'Carousel', 'cinerama' )          => 'carousel',
							esc_html__( 'Carousel Centered', 'cinerama' ) => 'carousel-centered'
						),
						'save_always' => true
					),
					array(
						...
						'heading'    => esc_html__( 'Number of Posts', 'cinerama' )
					),
					array(
						'type'        => 'dropdown',
						'param_name'  => 'orderby',
					...
					array(
						'type'        => 'autocomplete',
						'param_name'  => 'category',
						'heading'     => esc_html__( 'Category', 'cinerama' ),
						'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'cinerama' )
					),
					array(
						'type'       => 'dropdown',
						'param_name' => 'image_size',
						'heading'    => esc_html__( 'Image Size', 'cinerama' ),
						'value'      => array(
							...
							esc_html__( 'Thumbnail', 'cinerama' ) => 'thumbnail',
					...
					array(
						...
						'group'       => esc_html__( 'Post Info', 'cinerama' )
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Author', 'cinerama' ),
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Date', 'cinerama' ),
						'value'       => array_flip( cinerama_edge_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'cinerama' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Category', 'cinerama' ),
						'value'       => array_flip( cinerama_edge_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'cinerama' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Comments', 'cinerama' ),
	...
	public function render( $atts, $content = null ) {
		...
		cinerama_edge_get_module_template_part( 'shortcodes/blog-slider/holder', 'blog', '', $params );
    ...
    public function blogListCategoryAutocompleteSuggester( $query ) {
        ...
        if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
            foreach ( $post_meta_infos as $value ) {
                ...
                $data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'cinerama' ) . ': ' . $value['category_title'] : '' );
                $results[]     = $data;
            }
        }
        ...
        if ( ! empty( $query ) ) {
            ...
            if ( is_object( $category ) ) {
                ...
                if ( ! empty( $category_title ) ) {
                    $category_title_display = esc_html__( 'Category', 'cinerama' ) . ': ' . $category_title;
                }
