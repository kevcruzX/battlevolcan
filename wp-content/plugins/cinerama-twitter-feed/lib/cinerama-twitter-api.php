...
include_once 'cinerama-twitter-helper.php';

/**
 * Class CineramaTwitterApi
 */
class CineramaTwitterApi {
    /**
    ...
    const REQUEST_TOKEN_FIELD = 'cinerama_twitter_request_token';
    const REQUEST_TOKEN_SECRET_FIELD = 'cinerama_twitter_request_token_secret';
    const ACCESS_TOKEN_FIELD = 'cinerama_twitter_access_token';
    const ACCESS_TOKEN_SECRET_FIELD = 'cinerama_twitter_access_token_secret';
    const AUTHORIZE_TOKEN_FIELD = 'cinerama_twitter_authorize_token';
    const AUTHORIZE_VERIFIER_FIELD = 'cinerama_twitter_authorize_verifier';
    const USER_ID_FIELD = 'cinerama_twitter_user_id';
    const USER_SCREEN_NAME_FIELD = 'cinerama_twitter_screen_name';
    ...
    public function __construct() {
        ...
        $this->helper = new CineramaTwitterHelper();
    }
    /**
     ...
     *
     * @see CineramaTwitterApi::generateSignatureBase()
     */
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = esc_html__('Internal WP error', 'cinerama-twitter-feed');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Redirect URL couldn\'t be generated', 'cinerama-twitter-feed');
                    } else {
                        $responseObj->status = true;
                        $responseObj->message = esc_html__('Ok', 'cinerama-twitter-feed');
                    }
                } else {
                    $responseObj->status = false;
                    $responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'cinerama-twitter-feed');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = esc_html__('Access token obtained', 'cinerama-twitter-feed');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = esc_html__('Authorize token and it\'s secret were not obtained', 'cinerama-twitter-feed');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'cinerama-twitter-feed');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = esc_html__('It seems like you haven\'t connected with your Twitter account', 'cinerama-twitter-feed');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = esc_html__('Couldn\'t retrieve content from database', 'cinerama-twitter-feed');
            }
        }
